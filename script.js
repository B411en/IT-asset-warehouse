...
const firebaseConfig = {
        ...
        function openWarehouseModal() { document.getElementById('warehouse-modal').classList.remove('hidden'); document.getElementById('warehouse-modal').classList.add('flex'); }
        ...
        async function initApp() {
            ...
            if(accessState === 'unknown') {
                if(errorMsg) {
                    errorMsg.innerHTML = `<i class="fa-solid fa-triangle-exclamation mr-1"></i> Can't reach the server to verify the passcode. Check your connection and reload.`;
                    errorMsg.classList.remove('hidden');
                }
                if(pinInput) pinInput.setAttribute('disabled', 'true');
                return;
            }
